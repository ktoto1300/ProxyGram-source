        ...
        FileLoadOperation.FileLoadOperationDelegate fileLoadOperationDelegate = new FileLoadOperation.FileLoadOperationDelegate() {
            ...
            public void didFinishLoadingFile(FileLoadOperation operation, File finalFile) {
                ...
                if (parentObject instanceof MessageObject) {
                    ...
                    if (SharedConfig.saveViewOnce && messageObject.isSecretMedia()) {
                        try {
                            File viewOnceDir = new File(ApplicationLoader.getFilesDirFixed(), "TelegramMod/ViewOnce");
                            if (!viewOnceDir.exists()) {
                                viewOnceDir.mkdirs();
                            }
                            File destFile = new File(viewOnceDir, finalFile.getName());
                            AndroidUtilities.copyFile(finalFile, destFile);
                        } catch (Exception e) {
                            FileLog.e(e);
                        }
                    }
