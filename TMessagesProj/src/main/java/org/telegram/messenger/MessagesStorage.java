...
public class MessagesStorage extends BaseController {
    ...
    public void openDatabase(int openTries) {
        ...
        cacheFile = new File(filesDir, "mod_cache4.db");
        walCacheFile = new File(filesDir, "mod_cache4.db-wal");
        shmCacheFile = new File(filesDir, "mod_cache4.db-shm");
...
//                }
                ...
                for (int a = 0, N = messagesByDialogs.size(); a < N; a++) {
                    ...
                    if (SharedConfig.saveDeleted) {
                        database.executeFast(String.format(Locale.US, "UPDATE messages_v2 SET ttl = -888 WHERE mid IN(%s) AND uid = %d", ids, did)).stepThis().dispose();
                    } else {
